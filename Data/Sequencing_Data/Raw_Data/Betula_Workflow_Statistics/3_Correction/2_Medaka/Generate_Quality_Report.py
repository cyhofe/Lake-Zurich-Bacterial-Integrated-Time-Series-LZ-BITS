...
for filename in mapped_reads_files:
    ...
    mapped_reads_match = re.search(r'^(\d+)', lines[6])  # Fitth line contains mapped reads
    mapped_reads = int(mapped_reads_match.group(1)) if mapped_reads_match else "NA"
    row.append(mapped_reads)

    # Extract percent mapped
    percent_match = re.search(r'\((\d+)\.\d+%', lines[6])  # Fifth line contains percentage mapped
...
# -------- Combine DataFrames --------
# Join seqstat_df and mapped_reads_df on "Sample"
combined_df = pd.merge(seqstat_df, mapped_reads_df, on="Sample", how="outer")

# File path to save the combined DataFrame
output_file = os.path.join(directory_path, "Correction_2_Medaka_QualityReport.txt")
