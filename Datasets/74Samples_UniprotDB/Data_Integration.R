...
setwd("/SynologyDrive/PhD/Projects/SpringBloom2021/Datasets/74Samples_UniprotDB/")

#load data
Species_Dat <- read.csv("../../Data/Species_Data/Final_Data/Species_Table.csv")
Peptide_Dat <- read.csv("../../Data/Proteomics_Data/Final_Data/74Samples_uniprotkb_taxonomy_id_2_AND_reviewed_tr_2024_12_20_ContaminantsAdded_Database_Run11FEB25//Peptide_Table_Combined_Filtered.csv")
Protein_Dat <- read.csv("../../Data/Proteomics_Data/Final_Data/74Samples_uniprotkb_taxonomy_id_2_AND_reviewed_tr_2024_12_20_ContaminantsAdded_Database_Run11FEB25//Protein_Table_Combined_Filtered.csv")
...
Integrated_Gene_Table <- Gene_Dat %>%
  left_join(Protein_Dat, by = c("Species_ID", "Protein_ID", "Protein_Length")) %>%
  left_join(Species_Dat %>% select(Date, Species_ID, Species_Abundance, Relative_Species_Abundance_Per_Date, Total_Species_Abundance_Per_Date, Phylum, Class, Order, Family, Genus, Species), by = c("Date", "Species_ID")) %>%
  left_join(Lake_Dat %>% select(Date, Bacteria_CellsPerMl_Epilimnion) %>% distinct(), by = "Date") %>%
  left_join(MAG_Dat %>% filter(MAG_ID == Species_ID) %>% select(Species_ID, Contamination, Completeness, Estimated_Genome_Size) %>% distinct(), by = "Species_ID") %>%
  distinct()
...
# Sanity check
Summary_Integrated_Gene_Table_1 <- Integrated_Gene_Table %>%
  select(Protein_ID, Nr_Peptides) %>%
  distinct() %>%
  summarize(Nr_Unique_Peptides = sum(Nr_Peptides, na.rm = TRUE))
...
Summary_Integrated_Gene_Table <- cbind(Summary_Integrated_Gene_Table_1, Summary_Integrated_Gene_Table_2, Summary_Integrated_Gene_Table_3)
rm(Summary_Integrated_Gene_Table_1, Summary_Integrated_Gene_Table_2, Summary_Integrated_Gene_Table_3)
